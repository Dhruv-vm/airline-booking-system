...
def fetch_data1(name1, phone1):
    try:
        with open('boarding pass details.csv', 'r', newline='') as csvfile:
            ...
            for row in reader:
                if row['passenger_name'] == name1 and row['passport_no'] == phone1:
                    return row
...
def por_output(b,c,e):
    ...
    with open(DATA_FILE3, mode="a", newline="") as file:
        ...
        writer.writerow([passenger_name, departure_t, departure_tt, gate, bar, phoneno,seat,from1,from2,airline1])
...
def fetch_data(name, phone):
    try:
        with open('bookings.csv', 'r', newline='') as csvfile:
            reader = csv.DictReader(csvfile)
            for row in reader:
                if row['name'] == name and row['passport_no'] == phone:
                    return row
    except FileNotFoundError:
        print("CSV file 'bookings.csv' not found.")
...
def immik():
    ...
    tk.Label(immi, text="passport no:").pack()
    global phone_entry
...
def check_immigration():
    ...
    if os.path.exists("bookings.csv"):
        with open("bookings.csv", "r") as file:
            ...
            for row in reader:
                if row[3] == place_of_visit and row[0]== name and row[6]== phone:
...
#booking form part-1     
def place_booking(name_entry,place_var,phone_entry,passp_entry):
    global name,place,seat_entry,phoneno,email
    name= name_entry.get()
    place=place_var.get()
    phoneno=phone_entry.get()
    passportno=passp_entry.get()
    #["DUBAI", "SINGAPORE", "JAPAN","KOREA","SAUDI ARABIA","SWIZTERLAND"]
    if place=="DUBAI":
        ...
        save_button3 = tk.Button(booking_window, text="Save Booking", command=lambda:save_booking(name,place,flight_var,seats_entry,phoneno,passp_entry))
        ...
        save_button3 = tk.Button(booking_window, text="Save Booking", command=lambda:save_booking(name,place,flight_var,seats_entry,phoneno,passportno))
        save_button3.pack(pady=5)
...
def save_booking(name,place,flight_var,seats_entry,phoneno,passportno):
    ...
    with open(DATA_FILE, mode="a", newline="") as file:
        writer = csv.writer(file)
        writer.writerow([name, flight, seat, place, total, phoneno,passport])
...
def create_booking_window():
    ...
    passp_label = tk.Label(booking_window, text="passport no:")
    passp_label.pack(pady=5)
    passp_entry = tk.Entry(booking_window)
    passp_entry.pack(pady=5)
    ...
    save_button1 = tk.Button(booking_window, text="confirm destination", command=lambda:place_booking(name_entry,place_var,phone_entry,passp_entry))
